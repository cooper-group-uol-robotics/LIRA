...
/***
 * A utility class that allows to calibrate the saved frames using six point calibration.
 * 
 *
 */
public class CubeFinderAngled 
{
	...
	private Frame measurePoint(ObjectFrame tcp_frame, Frame estimation,
		...
		if (DEBUG) 
			_logger.info("Go To Point");

		tcp_frame.move((lin(p).setCartVelocity(30)));

		if (DEBUG)
			_logger.info("Measure");
		...
		IMotionContainer collision = tcp_frame.move(linRel(x, y, z, 0, 0, 0, tcp_frame)
			...
			if (collision.getFiredBreakConditionInfo() == null) {
				...
				if (DEBUG)
					_logger.info("detect collision: " + lbr_iiwa.getCurrentCartesianPosition(tcp_frame,
...
//		tcp_frame.move(positionHold(relaxMode, 300, TimeUnit.MILLISECONDS));
		...
		if (DEBUG)
			_logger.info("back from");
		tcp_frame.moveAsync(lin(p).setCartVelocity(30).setBlendingRel(0.5));
		return f;
	}
